...
export const QueuedPage: React.FC = () => {
    const { records, pageCount, currentPage, isFetching, render} = new TableRenderer<QueuedEntry>(Url.Queued);
                            ...
                            {records.get().slice((currentPage.get() - 1) * 10, currentPage.get() * 10).map((record, index) => (
                                <Table.Row key={`row-${index}`}>
                                    ...
                                    <Table.Cell textAlign="center">
                                        <i className="fas fa-trash btn-delete" onClick={() => { deleteRecord(record._id) }} />
                                    </Table.Cell>
                                </Table.Row>
                            ))}
                        </Table.Body>
