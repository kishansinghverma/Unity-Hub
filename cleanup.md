...
- `e81ee1f` — Oakter device synchronization and the frontend refresh action.
  ...
  - Added persistence to `src/backend/static/oak-devices.json`.
...
- `src/backend/static/oak-devices.json`
  - Ignored by Git.
...
- Remove `src/backend/static/oak-devices.json` only if the cached device catalog should be discarded.
